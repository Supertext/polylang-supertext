<script type="text/html" id="tmpl-sttr-order-link-row">
  <tr class="sttr-order-link-row">
    <td>&nbsp;</td>
    <td><img src="<?php echo SUPERTEXT_RESOURCE_URL .'/images/arrow-right.png'; ?>" width="16" height="16"/></td>
    <td colspan="2"><a href="#" onclick="Supertext.Interface.openOrderForm('{{data.targetLanguageCode}}')">&nbsp;<?php _e('Order translation', 'supertext'); ?></a></td>
  </tr>
</script>
  ...
  <div id="sttr-modal" class="sttr-modal">
    ...
    <div class="sttr-modal-content wp-core-ui">
      ...
      <div class="sttr-modal-header">
        <div class="logo"><img src="<?php echo SUPERTEXT_RESOURCE_URL . '/images/logo_supertext.png'; ?>" width="32" height="32" alt="Supertext" title="Supertext" /></div>
        <h1>{{data.title}}</h1>
...
<script type="text/html" id="tmpl-sttr-order-progress-bar">
  <div id="sttr-order-progress-bar" class="sttr-order-progress-bar">
    <ul>
      <li><?php _e('Select content and language', 'supertext'); ?></li>
      <li><?php _e('Select service and deadline', 'supertext'); ?></li>
      <li><?php _e('Confirmation', 'supertext'); ?></li>
    </ul>
    <div class="clearfix"></div>
    <hr />
  </div>
...
<script type="text/html" id="tmpl-sttr-content-step">
  <form id="sttr-content-step-form">
    <h2><?php _e('Content to be translated', 'supertext'); ?></h2>
    <p>
      <?php
      $adminUrl = get_admin_url(null, 'options-general.php?page=supertext-settings&tab=translatablefields');
      printf(wp_kses(
          ...
          <# _.each(data.posts, function(post) { #>
            <li class="{{post.meta.inTranslation ? 'hasError' : ''}}">
              <a href="#sttr-order-content-selection-{{post.id}}" data-post-id="{{post.id}}">
                ...
                {{post.title}} ({{post.languageCode}}{{post.meta.inTranslation ? ', <?php _e('blocked', 'supertext'); ?>' : ''}}{{post.meta.inProofreading ? ', <?php _e('blocked', 'supertext'); ?>' : ''}}{{post.isDraft ? ', <?php _e('draft', 'supertext'); ?>' : ''}})
              </a>
            </li>
          <# }); #>
        </ul>
        ...
        <# _.each(data.posts, function(post) { #>
          <div id="sttr-order-content-selection-{{post.id}}" style="display: none;">
            ...
            <p class="notice notice-error">
              <span class="error-message"><?php _e('The post cannot be translated. It is blocked by a translation order in progress.', 'supertext');?></span>
            </p>
            <# }else if(post.isDraft){ #>
            <p class="notice notice-warning">
              <span><?php _e('The post status is <b>draft</b>. Are you sure you want to order a translation for this article?', 'supertext');?></span>
            </p>
            <# } #>
            <p>
              <?php _e('Please select the content to be translated.', 'supertext');?>
            </p>
            <# _.each(post.translatableFieldGroups, function(translatableFieldGroup, groupId) { #>
              <h2>{{translatableFieldGroup.name}}</h2>
                ...
                <# } else { #>
                  - <?php _e('Not present in this post', 'supertext');?>
                <# } #>
                <div class="clearfix"></div>
            <# }); #>
          </div>
    ...
    <h2><?php _e('Language', 'supertext');?></h2>
    <p>
      <?php _e('Translate from <b id="sttr-order-source-language-label">-</b> into ', 'supertext');?>
      ...
      <select name="orderTargetLanguage" id="sttr-order-target-language">
        <option value=""><?php _e('Please select', 'supertext');?>...</option>
  ...
  <form id="sttr-content-step-form-pr">
    ...
    <div class="sttr-order-list">
      <div class="sttr-order-items">
        <ul>
          <# _.each(data.posts, function(post) { #>
          <li class="{{post.meta.inProofreading ? 'hasError' : ''}}">
            <a href="#sttr-order-content-selection-{{post.id}}" data-post-id="{{post.id}}">
              <span class="dashicons dashicons-no-alt"></span>
              {{post.title}} ({{post.languageCode}}{{post.meta.inProofreading ? ', <?php _e('blocked', 'supertext'); ?>' : ''}}{{post.isDraft ? ', <?php _e('draft', 'supertext'); ?>' : ''}})
            </a>
          </li>
          <# }); #>
        </ul>
      ...
      <div class="sttr-order-item-details">
        ...
        <div id="sttr-order-content-selection-{{post.id}}" style="display: none;">
          ...
          <# }else if(post.isDraft){ #>
          <p class="notice notice-warning">
            <span><?php _e('The post status is <b>draft</b>. Are you sure you want to order a proofread for this article?', 'supertext');?></span>
          </p>
          <# } #>
          <p>
