...
class AjaxRequestHandler
{
  ...
  public function getPostTranslationInfoAjax()
  {
    ...
    foreach ($postIds as $postId) {
      ...
      $postLang = $this->library->getMultilang()->getPostLanguage($postId);
      $translationInfo[] = array(
        'id' => $postId,
        'title' => $post->post_title,
        'languageCode' => $postLang === false ? substr(get_locale(), 0, 2) : $postLang,
        'meta' => array_merge(
          TranslationMeta::of($postId)->get(array(
            TranslationMeta::IN_TRANSLATION,
            TranslationMeta::SOURCE_LANGUAGE_CODE
          )),
          ProofreadMeta::of($postId)->get(array(
  ...
  public function sendSyncRequestAjax()
  {
    try {
      ...
      self::returnResponse(200, array(
        'message' => __('The changes have been sent successfully.', 'supertext')
      ));
    } catch (\Exception $e) {
      self::returnResponse(500, $e->getMessage());
    }
  ...
  private function processTargetPosts($order, $sourcePostIds, $targetPostIds, $sourceLanguage, $targetLanguage, $referenceHashes, $metaData, $syncTranslationChanges)
  {
    foreach ($sourcePostIds as $sourcePostId) {
      ...
      $message = sprintf(
        __('Translation order into %s has been placed successfully. Your order number is %s.', 'supertext'),
  ...
  public function sendSyncRequest($targetPost)
  {
    ...
    if (empty($oldTranslatableContent)) {
      throw new \Exception(__('Could not retrieve old version', 'supertext'));
    }
    ...
    Wrapper::sendSyncRequest(
      ...
      $this->library->toSuperCode($this->library->getMultilang()->getPostLanguage($targetPostId)),
  ...
  private function processProofreadPosts($order, $sourcePostIds, $sourceLanguage, $referenceHashes, $metaData){
    foreach ($sourcePostIds as $sourcePostId) {
      ...
      $meta = ProofreadMeta::of($proofreadPost->ID);
      $meta->set(ProofreadMeta::PROOFREAD, true);
      $meta->set(ProofreadMeta::IN_PROOFREADING, true);
      $meta->set(ProofreadMeta::IN_PROOFREADING_REFERENCE_HASH, $referenceHashes[$sourcePostId]);
  ...
  private function getLanguageName($polyCode)
  {
    ...
    return __($superCode, 'supertext-langs');
  }
  ...
  private function getUnfinishedTranslations($sourcePostId)
  {
    ...
    $languages = $this->library->getMultilang()->getLanguages();
    foreach ($languages as $language) {
      $targetPostId = $this->library->getMultilang()->getPostInLanguage($sourcePostId, $language->slug);
